...
void PE::run()
{
    ...
    while(1)
    {
        ...
        else if(i_setup->read())
        {
            ...
            {
                if(layer.PE_id == 0) p_max = layer.o.C; // bias
                else                 p_max = layer.o.H * layer.o.W;       
            }
        ...
        {
            if(i_cnt > i_max) cout << "<Error> Layer " << layer.layer_id << "[" << setw(2) << layer.PE_id << "] input data overflow." << endl;
            if(w_cnt > w_max) cout << "<Error> Layer " << layer.layer_id << "[" << setw(2) << layer.PE_id << "] weight data overflow." << endl;
            if(p_cnt > p_max) cout << "<Error> Layer " << layer.layer_id << "[" << setw(2) << layer.PE_id << "] psum data overflow." << endl;
